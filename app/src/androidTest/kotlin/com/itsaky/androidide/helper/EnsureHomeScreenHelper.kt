...
private fun detectPostOnboardingScreen(
	d: UiDevice,
	pkg: String,
	getStartedId: String,
...
private fun isGetStartedVisible(
	d: UiDevice,
	getStartedId: String,
): Boolean {
	...
	return byText.waitForExists(2000) &&
		byText.exists() &&
		runCatching { byText.visibleBounds }.getOrNull()?.let { it.width() > 4 && it.height() > 4 } == true
}

private fun closeProjectViaDrawerThenCloseWithoutSaving(
	d: UiDevice,
	context: Context,
) {
 ...
 */
fun waitForMainHomeOrEditorUi(
	device: UiDevice,
	maxWaitMs: Long = 900_000L,
) {
