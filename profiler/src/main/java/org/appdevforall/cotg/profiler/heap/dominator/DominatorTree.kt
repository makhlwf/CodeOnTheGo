...
internal class DominatorTree(
	 ...
	 */
	fun updateDominatedAsRoot(objectId: Long): Boolean = updateDominated(objectId, ValueHolder.NULL_REFERENCE)
	...
	fun buildFullDominatorTree(computeSize: (Long) -> Int): Map<Long, DominatorNode> {
		...
		val retainedSizes =
			computeRetainedSizes(allReachableObjectIds) { objectId ->
					...
					if (value != ValueHolder.NULL_REFERENCE) {
						...
						while (dominator != ValueHolder.NULL_REFERENCE) {
							...
							if (nodeRetainedSizes.containsKey(dominator)) {
								...
								val (currentRetainedSize, currentRetainedCount) =
									nodeRetainedSizes.getValue(
										dominator,
									)
